...
public class PokemonServidor {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println("Entrenador Gary quiere luchar. ¿Aceptas? (Y/N)");
			String respuesta = sc.nextLine();

			if (respuesta.equalsIgnoreCase("N")) {
				oos.writeObject(pokemon1);
				System.out.println("Eres un cagao");
				ois.close();
				oos.close();
				cliente.close();
			...
			if (respuesta.equalsIgnoreCase("Y")) {
				...
				if (numeroAzar < 50) {
					...
					pokemon1.setVida(0);
					oos.writeObject(pokemon1);
					System.out.println("El numero que ha salido es el " + numeroAzar + ". Has perdido"
							+ "\nPokemon: "+pokemon1.getNombre()+", vida: "+pokemon1.getVida());
					ois.close();
					oos.close();
					cliente.close();
				} else {
					...
					oos.writeObject(pokemon1);
					System.out.println("El numero que ha salido es el " + numeroAzar + ". Has ganado."
							+ "\nPokemon: "+pokemon1.getNombre()+", nivel: "+pokemon1.getNivel());
					ois.close();
					oos.close();
					cliente.close();
				}
